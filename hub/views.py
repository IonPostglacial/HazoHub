...
from . import utils
from .models import Character, Dataset, FileSharing, Hierarchy, State, Taxon, TaxonState
...
@login_required
def characters_list(req: HttpRequest, file_name: str, in_character: int = 0):
    ...
    if dataset is None:
        raise Http404(f"There is no dataset named {file_path}")
    selected_states = State.objects.filter(item_id__in=selected_states_ids).values('item_id', 'item__name')
    ...
    return render(req, 'hub/list_characters.html', { 
        'file_name': file_name,
...
def states_list(req: HttpRequest, file_name: str, in_character: int):
    ...
    return render(req, 'hub/select_states.html', { 
        'states': states,
        'file_name': file_name,
